...
void UTulExperienceManagerComponent::ServerSetCurrentExperience(FPrimaryAssetId ExperienceId)
{
	UTulAssetManager& AssetManager = UTulAssetManager::Get();

	TSubclassOf<UTulExperienceDefinition> AssetClass;
	{
	...
	// 왜 CDO를 가져오는 걸까?
	const UTulExperienceDefinition* Experience = GetDefault<UTulExperienceDefinition>(AssetClass);
...
void UTulExperienceManagerComponent::StartExperienceLoad()
{
	check(CurrentExperience);
	check(LoadState == ETulExperienceLoadState::Unloaded);

	LoadState = ETulExperienceLoadState::Loading;
	...
	//		1. 우리는 B_TulDefaultExperience를 BP로 만든 이유
	...
	for (const TObjectPtr<UTulExperienceActionSet>& ActionSet : CurrentExperience->ActionSets)
	{
		if (ActionSet)
		{
			// 앞서, 우리가 생성한 TUL_Shooter_SharedHUD가 추가되겠다 (물론 추가적인 TUL_Shooter_XXX)도 추가될거다
			// - BundleAssetList는 Bundle로 등록할 Root의 PrimaryDataAsset을 추가하는 과정이다
			// (-> ??? 무슨말인가 싶을건데 ChangeBundleStateForPrimaryAssets)를 살펴보면서 이해하자
	...
	// - Bundle이라는게 후일 우리가 로딩할 애셋의 카테고리 이름이라고 생각하면 된다 (일단 지금은 넘어가자 후일, 또 다룰 것이다!)
	TArray<FName> BundlesToLoad;
	{
		// 여기서 주목해야할 부분은 OwnerNetmode가 NM_Standalone이면? Client/Server 둘 다 로딩에 추가된다!
	...
	// 아래도, 후일 Bundle을 우리가 GameFeature에 연동하면서 더 깊게 알아보기로 하고, 지금은 앞서 B_TulDefaultExperience를 로딩해주는 함수로 생각하자
...
void UTulExperienceManagerComponent::OnExperienceLoadComplete()
{
