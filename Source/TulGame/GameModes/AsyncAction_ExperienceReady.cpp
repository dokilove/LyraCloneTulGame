...
void UAsyncAction_ExperienceReady::Activate()
{
	if (UWorld* World = WorldPtr.Get())
	{
		...
		{
			// 아직 준비되어 있지 않으면, UWorld::GameStateSetEvent에 Step1_HandleGameState를 Delegate로 바인딩시킨다
			// - Step1부터 시작할 것이다
	...
	else
	{
		...
		// - 코드를 내려가보면, GameInstance에서 레퍼런스 대상으로 제거 시킨다
...
void UAsyncAction_ExperienceReady::Step2_ListenToExperienceLoading(AGameStateBase* GameState)
{
	...
	if (ExperienceManagerComponent->IsExperienceLoaded())
	{
		...
		// ControllerComponent_CharacterParts가 설정하는 시점에는 Pawn이 Possess되지 않기 때문에 UPawnComponent_CharacterParts를 호출할 수 없어서 결론적으로 Mesh 설정이 불가능하다
	...
	else
	{
		...
		ExperienceManagerComponent->CallOrRegister_OnExperienceLoaded(FOnTulExperienceLoaded::FDelegate::CreateUObject(this, &ThisClass::Step3_HandleExperienceLoaded));
	}
}

void UAsyncAction_ExperienceReady::Step3_HandleExperienceLoaded(const UTulExperienceDefinition* CurrentExperience)
{
