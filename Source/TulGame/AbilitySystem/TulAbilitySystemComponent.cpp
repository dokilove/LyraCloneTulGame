...
void UTulAbilitySystemComponent::AbilityInputTagPressed(const FGameplayTag& InputTag)
{
	if (InputTag.IsValid())
	{
		...
		{
			// Ability가 존재하고, DynamicAbilityTags에 InputTag에 있을 경우, InputPressed/Held에 넣어 Ability 처리를 대시한다
...
void UTulAbilitySystemComponent::AbilityInputTagReleased(const FGameplayTag& InputTag)
{
	if (InputTag.IsValid())
	{
		for (const FGameplayAbilitySpec& AbilitySpec : ActivatableAbilities.Items)
...
void UTulAbilitySystemComponent::PrecessAbilityInput(float DeltaTime, bool bGamePaused)
{
	...
	for (const FGameplayAbilitySpecHandle& SpecHandle : InputHeldSpecHandles)
	{
		...
		if (const FGameplayAbilitySpec* AbilitySpec = FindAbilitySpecFromHandle(SpecHandle))
		{
			...
			{
				const UTulGameplayAbility* TulAbilityCDO = CastChecked<UTulGameplayAbility>(AbilitySpec->Ability);

				// ActivationPolicy가 WhileInputActive 속성이면 활성화로 등록
				if (TulAbilityCDO->ActivationPolicy == ETulAbilityActivationPolicy::WhileInputActive)
				{
		...
		if (FGameplayAbilitySpec* AbilitySpec = FindAbilitySpecFromHandle(SpecHandle))
		{
			if (AbilitySpec->Ability)
			{
				...
				else
				{
					...
					if (TulAbilityCDO->ActivationPolicy == ETulAbilityActivationPolicy::OnInputTriggered)
					{
						AbilitiesToActivate.AddUnique(AbilitySpec->Handle);
					}
				}
	...
	// 등록된 AbilitiesToActive를 한꺼번에 등록 시작:
	...
	// InputHeldSpecHandles은 InputReleasedSpecHandles 추가될 때 제거된다!
