...
#include "TulGameFeaturePolicy.generated.h"

/**
* UTulGameplayFeaturePolicy는 GameFeature Plugin이 Memory에 로딩되거나 Active(활성화)를 관리하는 StateMachine을 모니터링 가능하다
*/

UCLASS()
class UTulGameFeaturePolicy : public UDefaultGameFeaturesProjectPolicies
{
	GENERATED_BODY()
public:
	UTulGameFeaturePolicy(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	*/
	...
	/** Observer로서 등록한 객체를 관리 (아래  TulGameFeature_AddGameplayCuePaths가 하나의 예시 */
	UPROPERTY(Transient)
	TArray<TObjectPtr<UObject>> Observers;
};

/**
* GameFeature Plugin이 Register/Unregister 단계에서 GameplayCuePath가 등록되어 GameplayCueManager가 관련 GameplayCue 애셋을 스캔할 수 있도록 한다
...
* - 이를 위해 추가적 구현이 필요한데 이는 UTulGameplayFeaturePolicy를 참고하자
* 
...
*/
UCLASS()
class UTulGameFeature_AddGameplayCuePaths : public UObject, public IGameFeatureStateChangeObserver
{
	GENERATED_BODY()
public:
	/**
