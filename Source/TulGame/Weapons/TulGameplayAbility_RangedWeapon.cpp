...
#include "TulGameplayAbility_RangedWeapon.h"
#include "CollisionQueryParams.h"
#include "CollisionShape.h"
#include "DrawDebugHelpers.h"
#include "TulRangedWeaponInstance.h"
#include "GameFramework/Pawn.h"
#include "GameFramework/PlayerController.h"
#include "TulGame/Physics/TulCollisionChannels.h"
#include "Engine/World.h"
#include "TulGame/AbilitySystem/TulGameplayAbilityTargetData_SingleTarget.h"
#include "AbilitySystemComponent.h"

UTulGameplayAbility_RangedWeapon::UTulGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
}

void UTulGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	// - TulAbilitySet::GiveToAbilitySystem()에서 GiveAbility()를 호출한다
	...
	// GameplayAbilityTargetData는 Server/Client 간 Ability의 공유데이터로 이해하면 된다
	// - 허나, 우리는 싱글플레잉이므로 Ability의 데이터로 생각하면 되겠다. (현재 큰 의미가 없다고 볼 수 있다)
	FGameplayAbilityTargetDataHandle TargetData;
	TargetData.UniqueId = 0;

	if (FoundHits.Num() > 0)
	{
		// Cartridge란 일반 권총의 경우, 탄약에 하나의 총알이 들어있지만, 샷건의 경우, 탄약에 여러개의 총알이 있고, ** 탄약을 카트리지로 생각** 하면된다
		const int32 CartridgeID = FMath::Rand();
		for (const FHitResult& FoundHit : FoundHits)
		{
			// AbilityTargetData에 SingleTargetHit 정보를 담는다
			// - 참고로  TargetData.Add()의 경우, SharedPtr에 넣기 때문에 여기서 new는 크게 신경 안써도 된다
			FTulGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FTulGameplayAbilityTargetData_SingleTargetHit();
...
void UTulGameplayAbility_RangedWeapon::PerformLocalTargeting(TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	UTulRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, ETulAbilityTargetingSource::CameraTowardsFocus);
...
FTransform UTulGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, ETulAbilityTargetingSource Source)
{
	check(SourcePawn);
	check(Source == ETulAbilityTargetingSource::CameraTowardsFocus);
	...
	// PlayerController 로 부터, Location과 Rotation 정보를 가져옴
...
FVector UTulGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
void UTulGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, TArray<FHitResult>& OutHits)
{
	UTulRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	// 총알을 하나 Trace 진행한다:
	// - 참고로 Lyra의 경우, 샷건 과 같은 Cartridge에 여러개의 총알이 있을 경우를 처리하기 위해 for-loop를 활용하여, 복수개  Bullet을 trace한다
...
FHitResult UTulGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, TArray<FHitResult>& OutHits) const
{
	FHitResult Impact;

	// 우선 SweepRadius 없이 한번 Trace 진행한다 (SweepTrace는 무겁기 때문)
	// - FindFirstPawnHitResult()를 여러번 Trace진행을 막기 위해, OutHits를 확인해서 APawn 충돌 정보 있으면 더 이상 Trace하지 않는다
		...
		if (SweepRadius > 0.0f)
		{
			...
			if (SweepHits.IsValidIndex(FirstPawnIdx))
			{
				...
				for (int32 Idx = 0; Idx < FirstPawnIdx; ++Idx)
				{
					...
					// - OutHits에 bBlockingHit가 SweepHits 로 있음을 알게 되었음
...
FHitResult UTulGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, TArray<FHitResult>& OutHitResults) const
{
...
void UTulGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UTulGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return Tul_TraceChannel_Weapon;
}

void UTulGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
...
UTulRangedWeaponInstance* UTulGameplayAbility_RangedWeapon::GetWeaponInstance()
{
	return Cast<UTulRangedWeaponInstance>(GetAssociatedEquipment());
}
